...
describe('api calls', () => {
  ...
  describe('the getFeedbackRequests operation', () => {
    it('gets a list of feedback requests', async () => {
      ...
      const feedbackRequestList = await getFeedbackRequests();
      expect(graphqlOperation.mock.calls.length).toEqual(1);
  ...
  describe('the removeFeedbackRequest operation', () => {
    it('deletes a feedback request', async () => {
      const deleteFeedbackRequestId = 1234;
      API.graphql.mockResolvedValue({data: {deleteFeedbackRequest: deleteFeedbackRequestId}});
      graphqlOperation.mockReturnValue('the delete feedback request mutation');
      const deletedID = await removeFeedbackRequest(deleteFeedbackRequestId);
      expect(graphqlOperation.mock.calls.length).toEqual(1);
      expect(graphqlOperation.mock.calls[0][0]).toContain('DeleteFeedbackRequest');
      expect(graphqlOperation.mock.calls[0][1]).toEqual({
    ...
    it('delete propogates the errors', async () => {
      ...
      graphqlOperation.mockReturnValue('the delete feedback request mutation');
      expect.assertions(1);
      return removeFeedbackRequest().catch(errorMessage => {
        expect(errorMessage).toEqual({error: 'some delete error'});
      });
    });
  });
